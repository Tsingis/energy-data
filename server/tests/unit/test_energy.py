...
class TestEnergy(unittest.TestCase):
    def setUp(self):
        ...
        self.last_time = datetime.now(tz=UTC) - timedelta(hours=1)
    ...
    def test_from_api_response_mapping(self):
        ...
        self.assertEqual(datetime(2025, 3, 20, 10, 0, 0, 0, tzinfo=UTC), energy.timestamp)
    ...
    def test_to_response_model(self):
        ...
        self.assertEqual(datetime(2025, 3, 20, 10, 0, 0, tzinfo=UTC), production_data[0].timestamp)
        self.assertEqual(100.0, production_data[0].value)
        self.assertEqual(datetime(2025, 3, 20, 11, 0, 0, tzinfo=UTC), production_data[1].timestamp)
        ...
        self.assertEqual(datetime(2025, 3, 20, 10, 0, 0, tzinfo=UTC), consumption_data[0].timestamp)
    ...
    def test_adjust_predictions_no_adjustment(self):
        data = {
            Dataset.PRODUCTION.key_name: [Energy(timestamp=datetime.now(tz=UTC), value=100.0)],
            Dataset.CONSUMPTION_PREDICTION.key_name: [
                Energy(timestamp=datetime.now(tz=UTC) + timedelta(hours=1), value=200.0)
            ],
        }
